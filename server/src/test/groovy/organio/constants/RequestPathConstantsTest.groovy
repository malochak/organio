...
class RequestPathConstantsTest extends Specification {
    def 'should throw AssertionError on new Constants event creations'() {
        when:
        new RequestPathConstants()

        then:
        def exception = thrown(AssertionError)
        exception.message == String.format(UNAUTHORIZED_ACCESS_FORMAT, 'organio.constants.RequestPathConstants')
    }
}
